...
namespace Anonym.Isometric
{
    ...
    public class IsometricMovement : MethodBTN_MonoBehaviour
    {
        ...
        public bool bContinuousMovement { get { return bSnapToGroundGrid == false; } }
        ...
        protected float fTickSpeed { get { return (bDashing ? fDashSpeedMultiplier : 1f) * fSpeed * Time.deltaTime; } }
        ...
        virtual public Collider cCollider {
            get
            {
        ...
        virtual public Vector3 vFeetPosition
        {
            get
            {
                return new Vector3(transform.position.x, GetBounds().min.y, transform.position.z);
            }
        }
        #endregion
        ...
        virtual protected bool GetTDFromPath(
            Vector3 startPosition, Vector3[] corners, 
            out Vector3 vTranslate, out Vector3 vDestination, 
            Vector3 vStepOffset = new Vector3())
        {
            ...
            for (int i = 1; i < corners.Length; ++i)
            {
                ...
                if (vTranslate.sqrMagnitude >= fSqrtMinMovement)
                {
                    ...
                    {
                        if (!RayTest(startPosition + vStepOffset, vTranslate))
                            vTranslate = HorizontalVector(Convert(vTranslate));
                        else if (RayTest(startPosition + vStepOffset, vX))
                            vTranslate = vZ;
                        else if (RayTest(startPosition + vStepOffset, vZ))
                            vTranslate = vX;
                        else
                            vTranslate = HorizontalVector(Convert(vTranslate));
                    }
                    else if (HorizontalRayCast(startPosition + vStepOffset, vTranslate))
                    {
        ...
        public Vector3 GetXZPositionWithSnapOption(Vector3 vPosition)
        {
            var vTmp = SnapPosition(vPosition, bSnapToGroundGrid);
            vTmp.y = vPosition.y;
            return vTmp;
        }
        ...
        virtual protected bool HorizontalRayCast(Vector3 startPosition, Vector3 vTranslate)
        {
            vTranslate.y = 0;
            return RayTest(startPosition, vTranslate);
        }

        virtual protected bool RayTest(Vector3 from, Vector3 vTranslate)
        {
            RaycastHit nHit;
            return Physics.Raycast(from, vTranslate, out nHit, vTranslate.magnitude, -1, QueryTriggerInteraction.Ignore);
        }
        ...
        protected static InGameDirection convert_4Dir(Vector3 vMoveTo)
        {
        ...
        protected static InGameDirection convert_8Dir(Vector3 vMoveTo)
        {
            ...
            if (x * z == 0)
            {
                ...
                else if (bNegativeX)   result = InGameDirection.LT_Move;
                else if (bPositiveZ)    result = InGameDirection.RT_Move;
                else if (bNegativeZ)   result = InGameDirection.LD_Move;
            } 
