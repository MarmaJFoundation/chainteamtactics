...
namespace Anonym.Isometric
{
    ...
    public class KeyInputAssist : Util.Singleton<KeyInputAssist>
    {
        ...
        [SerializeField, Header("Only available with NavMeshAgent & IsometricCharacterController")]
        ...
        void ClickToMove()
        {
            ...
            {
                if (NMAgent != null && NMAgent.ClickToMove(out destination))
                    AddAnchor(destination);

                if (target.TryGetComponent<SimplePathfinder>(out var pathfinder))
                {
                    if (pathfinder.ClickToMoveWithNavMesh(target.vFeetPosition))
                        AddAnchor(pathfinder.LastPosition());
                }
            }
        }
