...
namespace Anonym.Isometric
{
	...
	public class IsoMap : Singleton<IsoMap> {
        ...
        [SerializeField]
        IsoGrid _grid;
        public IsoGrid gGrid
        {
            get
            {
                if (_grid == null)
                    _grid = GetComponent<IsoGrid>();
                return _grid;
            }
        }
			...
			if (bUseIsometricSorting)
			{
				if (!bCustomResolution)
				{
					fResolutionOfIsometric.Set(
						Mathf.Max(IsoGrid.fGridTolerance, Mathf.Sin(Mathf.Deg2Rad * -TileAngle.y) * fResolution),
						Mathf.Max(IsoGrid.fGridTolerance, Mathf.Sin(Mathf.Deg2Rad * TileAngle.x) * fResolution),
						Mathf.Max(IsoGrid.fGridTolerance, Mathf.Cos(Mathf.Deg2Rad * -TileAngle.y) * fResolution)
					);
				}
				else
				{
